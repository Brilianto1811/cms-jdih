            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('agenda.update', $agenda) }}" method="POST">
                        ...
                        <!-- Tanggal Kegiatan -->
                        <div class="mb-4">
                            <label for="tgl_agenda" class="text-lg font-medium">Tanggal Kegiatan</label>
                            <input type="date" name="tgl_agenda" id="tgl_agenda"
                                class="border-gray-300 shadow-sm w-full rounded-lg"
                                value="{{ old('tgl_agenda', $agenda->tgl_agenda) }}" required>
                            @error('tgl_agenda')
                            ...
                            @foreach ($agenda->agendaKegiatan as $index => $kegiatan)
                                <div class="kegiatan-item mb-4 border-b border-gray-300 pb-4">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <input type="text"
                                            name="agenda_kegiatan[{{ $index }}][judul_kegiatan]"
                                            class="border-gray-300 shadow-sm w-full rounded-lg"
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <textarea name="agenda_kegiatan[{{ $index }}][deskripsi_kegiatan]"
                                            class="border-gray-300 shadow-sm w-full rounded-lg" required>{{ old("kegiatan.$index.deskripsi_kegiatan", $kegiatan->deskripsi_kegiatan) }}</textarea>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        let kegiatanCount = {{ $agenda->agendaKegiatan->count() }};
        ...
        document.getElementById('add-kegiatan').addEventListener('click', function() {
            ...
            const newKegiatan = `
                <div class="kegiatan-item mb-4 border-b border-gray-300 pb-4">
                    <div class="mb-2">
                        <label class="text-lg font-medium">Waktu Kegiatan</label>
                        <input type="time" name="agenda_kegiatan[\${kegiatanCount}][waktu_kegiatan]" class="border-gray-300 shadow-sm w-full rounded-lg" required>
                    </div>
                    <div class="mb-2">
                        <label class="text-lg font-medium">Judul Kegiatan</label>
                        <input type="text" name="agenda_kegiatan[\${kegiatanCount}][judul_kegiatan]" class="border-gray-300 shadow-sm w-full rounded-lg" required>
                    </div>
                    <div class="mb-2">
                        <label class="text-lg font-medium">Deskripsi Kegiatan</label>
                        <textarea name="agenda_kegiatan[\${kegiatanCount}][deskripsi_kegiatan]" class="border-gray-300 shadow-sm w-full rounded-lg" required></textarea>
                    </div>
