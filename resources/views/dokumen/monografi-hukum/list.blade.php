<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            @can('create monografi-hukum')
                <a href="{{ route('monografi-hukum.create') }}"
                ...
                <table class="min-w-full table-auto">
                    ...
                    <tbody class="bg-white">
                        @if ($monografi->isNotEmpty())
                            @foreach ($monografi as $items)
                                <tr class="border-b">
                                    <td class="px-6 py-4 text-center whitespace-nowrap">
                                        @can('edit monografi hukum')
                                            <a href="{{ route('monografi-hukum.edit', $items->id) }}"
                                                class="bg-slate-700 text-sm rounded-md text-white px-3 py-2 hover:bg-slate-600">Edit</a>
                                        @endcan

                                        @can('delete monografi hukum')
                                            <a href="javascript:void(0)" onclick="deleteMonografi({{ $items->id }})"
    ...
    <x-slot name="script">
        <script type="text/javascript">
            window.deleteMonografi = function(id) {
                if (confirm("Are you sure you want to delete?")) {
                    $.ajax({
                        url: '{{ route('monografi-hukum.destroy') }}',
                        type: 'DELETE',
                        data: {
                            id: id,
