    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                Struktur Organisasi / Buat
            </h2>
            <a href="{{ route('struktur-organisasi.list') }}"
                class="bg-slate-700 text-sm rounded-md text-white px-5 py-2">
                Kembali
            </a>
        </div>
    </x-slot>
                ...
                <form action="{{ route('struktur-organisasi.store') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input value="{{ old('jabatan') }}" name="jabatan" placeholder="Jabatan" type="text"
                                class="border-gray-300 shadow-sm w-full rounded-lg p-2">
                            @error('jabatan')
                                <p class="text-red-400 font-medium">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label for="urutan" class="text-lg font-medium">Urutan</label>
                            <input value="{{ old('urutan') }}" name="urutan" placeholder="Urutan" type="number"
                                class="border-gray-300 shadow-sm w-full rounded-lg p-2">
                            @error('urutan')
                                <p class="text-red-400 font-medium">{{ $message }}</p>
                            @enderror
